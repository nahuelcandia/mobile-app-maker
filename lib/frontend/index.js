...
function frontend(app) {
  ...
  debug("These plugins are reachable to this module: %s",
    Object.keys(app.plugins).join(", "));
  app.use("/", express.static(templatePath));
  app.use("/coreassets", express.static(
    join(paths.adminTemplateBase, "assets")));

  frontend.hook("render", function(data, locals, next) {
    renderFrontend(app, locals, function(err, html) {
      data.html = html;
      next();
    });
  })
...
function frontendMiddleware(req, res) {
  ...
  frontend.trigger("render", {
    html: ""
  }, locals, function(err, data, locals) {
    if (err) {
      return res.send(err);
    }
    res.send(data.html);
  ...
  app.render("index.jade", locals, function(err, html) {
    if (err) {
      ...
      app.render("index.html", locals, function(err, html) {
        ...
        debug("Rendered frontend to express route successfully");
        return cb(null, html);
      });
    } else {
      //html = replaceEditables(html, updatedEditables);
