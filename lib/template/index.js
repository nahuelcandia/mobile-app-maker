 ...
 * - Handles static routes  admin/assets for the assets folder inside the admin template
 *
 * @param {express.App} express request handler
 */
function template(app, template) {
  // Set default engine. If not, user gets this message when
  // no extension is provided
  var template = (typeof template === 'undefined' ? options.currentTemplate : template);
