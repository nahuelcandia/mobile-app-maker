...
template.findEnabledTemplate = function(app, callback) {
  templatesdb.find({
    ...
    if (docs.length === 1) {
      if(app.locals.templates[docs[0].name]) {
        templateName = docs[0].name;
        debug("Found enabled template '%s' in database", docs[0].name);
      }else{
        debug("Found enabled template '%s', but it can't be found ...", docs[0].name);
        debug("... on '%s' directory. Maybe deleted? Using default: '%s'",
          config.get('paths.templatesBase'), config.get('template'));
        debug("You should either restore the missing template directory or select a valid one");
...
template.findInDirectory = function(app, callback) {
  ...
  try {
    walk.sync(join(process.cwd(), config.get('paths.templatesBase')), {
...
/**TESTING
 * Returns if the template exists and is "usable"
 * Checks if path exists and if it has, at least
 * an index.html or index.jade
 *
 * @param: {String} templateName: the (directory)name of the template
 */
function templateIsUsable(templateName) {
  var templatePath = join(process.cwd(),config.get('paths.templatesBase'),templateName);
  //check path exists
