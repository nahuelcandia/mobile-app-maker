...
module.exports = function(app) {
  ...
  app.cms.menus.registerAdminGroupMenu({
    title: 'Templates',
    description: 'Manage current templates or add a new one',
  ...
  app.post("/admin/template/edit/:name/:thefile", saveTemplate);
  ...
  app.post("/admin/template/get/:name", readTemplateFile);
  app.post("/admin/template/config/:name", setConfig);
};

function listTemplateDir(filename, template, level) {
  var ext = filename.split('.')[filename.split('.').length - 1];
  var extensions = ['css', 'html', 'js', 'less', 'jade', 'json'];
  var blacklisted_suffixes = ['.min.css', '.min.js'];
  var rootPath = process.cwd() + '/templates/' + template;
  ...
  if (isDirectory) {
    ...
    tree.branch = fs.readdirSync(filename).map(function(child) {
      return listTemplateDir(filename + '/' + child, template, level + 1);
    });
  } else {
    tree.icon = 'file';
    tree.inode = false;
...
function editTemplate(req, res) {
  ...
  templates.find({
    ...
    var tree = listTemplateDir(path, req.params.name, 0);
...
function saveTemplate(req, res, next) {
  ...
  req.on('end', function() {
    ...
    var data = content;
    fs.writeFile(process.cwd() + '/templates/' + req.params.name + '/' + req.params.thefile, data);
